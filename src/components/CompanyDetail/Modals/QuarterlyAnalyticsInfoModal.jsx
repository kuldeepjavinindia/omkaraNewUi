import { Button, Dialog, DialogBody, DialogHeader, DialogFooter, IconButton, Tooltip, Alert } from '@material-tailwind/react'
...
const QuarterlyAnalyticsInfoModal = (props) => {
  ...
  const {
    ...
    const getColorStatus = (val='', type='', isHide=true) => {
      ...
      return (
          <>
              { type === 'withStatus' && withStatus } <span style={{ color: val <= 0 ? '#DD2025' : '#34A853' }}>{val}{ isHide === true ? '%' : "" }</span>
          </>
      )
      ...
      <Dialog open={DialogData} handler={()=>setDialogData(null)}>
        ...
        <div id="dataToCopy" >
          ...
          <div ref={Ref} className=' font-medium text-[15px] text-black'>
            ...
            <div>
                <h4 className='text-black'><strong className=' font-bold text-[24px]'>{NewData?.company_name}</strong><span className=' font-medium'> {DialogData}</span></h4>
                <h6 className=' text-[13px] text-gray-400'>Omkara Results Review {SelectedQType?.label ? ' - '+SelectedQType?.label : ""}</h6>
            </div>
            <div className='flex gap-0.5'>
                <IconButton  onMouseEnter={() => setCopied(false)}   size='sm' className=' bg-transparent shadow-none' onClick={()=>onCopy()}>
                  <img className='h-5' src={`${import.meta.env.VITE_BASE_URL}/images/icons/copy_icon.svg`} alt="" />
                </IconButton>


                <IconButton size='sm' className=' bg-transparent shadow-none'>
                  <img className='h-5' src={`${import.meta.env.VITE_BASE_URL}/images/icons/wp_icon.svg`} alt="" />
                </IconButton>
            </div>
          </div>
            <br />
            ...
            <div className='text-black text-xl mb-6'><strong>Segmental Performance:-</strong></div>
        ...
        <DialogFooter className="space-x-2">
          <Button className='bg-theme' fullWidth onClick={()=>setDialogData(null)}>
            Go Back
          </Button>
